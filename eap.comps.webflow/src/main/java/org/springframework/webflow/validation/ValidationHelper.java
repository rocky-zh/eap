...
public class ValidationHelper {
	...
	public void validate() {
		...
		// add by chiknin@gmail.com
		MessageContext msgCtx = requestContext.getMessageContext();
//		msgCtx.hasErrorMessages()
		Message[] msgs = msgCtx.getMessagesByCriteria(new MessageCriteria() {
			@Override
		...
		if (msgs.length > 0) {
//			requestContext.getRequestScope().put("xxxxxxxxxxxxxxxxx", 1); // cross requsest
			Env env = EapContext.getEnv();
			requestContext.getViewScope().put(env.getProperty("webflow.validation.errorMessageKey", "wbf_validate_errors"), msgs);
		}
	}
